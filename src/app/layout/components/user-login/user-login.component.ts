  ...
  template: `
    @if (!authService.currentUser()) {
      <form (ngSubmit)="onLogin()" style="display: flex; gap: 0.5rem;">
        ...
        <button
          type="submit"
          style="padding: 0.5rem 1rem; background-color: #007bff; color: white; border: none; cursor: pointer;"
        >
          Iniciar sesión
        </button>
      </form>
      ...
      <button
        (click)="onLogout()"
        style="margin-left: 1rem; padding: 0.5rem 1rem; background-color: #dc3545; color: white; border: none; cursor: pointer;"
      >
        Cerrar sesión
      </button>
    }
